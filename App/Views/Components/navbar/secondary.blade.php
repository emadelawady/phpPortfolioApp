        ...
        <nav x-data="{ isOpen: false }" class="bg-white px-6 shadow dark:bg-gray-800">
            <div class="container px-6 py-3 mx-auto md:flex">
                <div class="flex items-center justify-between nav-item">
                    <a href="{{ \Core\Helper::route('homepage') }}" class="">
                ...
                <div x-cloak :class="[isOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full']" class="absolute inset-x-0 z-20 w-full px-6 py-4 transition-all duration-300 ease-in-out bg-white dark:bg-gray-800 md:mt-0 md:p-0 md:top-0 md:relative md:opacity-100 md:translate-x-0 md:flex md:items-center md:justify-between">
                    ...
                    @if ($_SESSION['user'] ?? false)
                        ...
                        <li class="list-none text-gray-700 transition-colors duration-300 transform dark:text-gray-200 {{ \Core\Helper::is_current_route('login') ? 'active' : '' }}">
                            <a href="{{ \Core\Helper::route('login') }}" class="px-2.5 py-2 md:mx-2">Login</a>
                        </li>
                        <li class="list-none text-gray-700 transition-colors duration-300 transform dark:text-gray-200 {{ \Core\Helper::is_current_route('register') ? 'active' : '' }}">
                            <a href="{{ \Core\Helper::route('register') }}" class="px-2.5 py-2 md:mx-2">Register</a>
                        </li>
                     @endif
                    </div>
                </div>
